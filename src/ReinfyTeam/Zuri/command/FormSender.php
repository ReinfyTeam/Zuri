...
final class FormSender extends ConfigManager {
	...
	public static function ToggleModules(Player $player) : void {
		$form = new SimpleForm(function(Player $player, $data) {
			if ($data === null) {
				return;
			}
	...
	public static function ChangeMaxVL(Player $player, Check $check) : void {
		...
		$form->addLabel("Modify the slider do you want to set..");
		$form->addSlider("MaxVL", 1, 100, -1, intval(self::getData(self::CHECK . "." . strtolower($check->getName()) . ".maxvl")));
		$player->sendForm($form);
	}

	public static function AdjustCodeLength(Player $player) : void {
	...
	public static function ModifySendType(Player $player) : void {
		$form = new CustomForm(function(Player $player, $data) {
			if ($data === null) {
				return;
			}
		...
		$form->addToggle("Randomize Send Type", self::getData(self::CAPTCHA_RANDOMIZE), null, "randomize");
		$player->sendForm($form);
	}
}